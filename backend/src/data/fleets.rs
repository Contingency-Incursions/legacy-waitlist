use crate::core::esi::{self, fleet_members::ESIFleetMember, ESIClient, ESIError, ESIScope};
...
pub async fn load_default_squads() -> Vec<Wing> {
...
pub async fn delete_all_wings(esi_client: &ESIClient, fleet: &FleetInfo) -> Result<(), ESIError> {
    let current_wings: Vec<WingInfo> = esi_client
        .get(
...
pub async fn set_default_motd(
    ...
    let base_motd_template =
        std::fs::read_to_string("./data/motd.dat").expect("Could not load motd.dat");

    let mut result = base_motd_template;

    if let Some(fc) = sqlx::query!(
    ...
    .await?
    {
        ...
        result = result.replace("{fc_name}", &fc.name.to_string());
    }

    let members: Vec<ESIFleetMember> =
        esi::fleet_members::get(esi_client, fleet.fleet_id, fleet.fleet_boss_id)
            .await?
            .into_iter()
            .collect();
    ...
    result = result.replace("{fc_system_name}", &boss_system_name.to_string());
