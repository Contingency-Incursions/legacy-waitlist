...
async fn fleet(
    ...
    let id_list: String = character_ids
        .iter()
        .map(|n| n.to_string())
        .collect::<Vec<String>>()
        .join(", ");
    ...
    .map(|squad| {
        (
            squad.squad_id,
            [squad.category, squad.wing_id.to_string()].to_vec(),
        )
    })
    .collect();
            ...
            position: FleetPosition {
                squad: match squads.get(&r.squad_id) {
                    ...
                    None => {
                        if r.squad_id == -1 {
                            "logi".to_string()
                        } else if &(r.wing_id.to_string()) == on_grid_wing {
                            "boxer".to_string()
                        } else {
                            "Off Grid".to_string()
                        }
                ...
                wing: match squads.get(&r.squad_id) {
                    Some(_) => "On Grid".to_string(),
                    None => {
                        if &r.wing_id.to_string() == on_grid_wing {
                            "On Grid".to_string()
                        } else {
                            "Off Grid".to_string()
                        }
                ...
                is_alt: match squads.get(&r.squad_id) {
                    Some(v) => v.get(0).unwrap() == "alt",
                    None => false,
                },
